...
const ActiveSprintProgressGraphComp = () => {
    const [loading, setLoading] = useState(true);
    const [data, setData] = useState([]);
    const fetchIssueStatues = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get("/taskboard-overview/active-sprint");
    ...
    return (
        <Grid size={3} sx={{ borderRadius: "8px", bgcolor: "#fcfafa", boxShadow: "0px 4px 8px rgba(0, 0, 0, 0.1)", marginY: "auto", padding: "5px" }}>
            ...
            <SprintProgressGraph data={data}/>
        </Grid>
    )
}

export default ActiveSprintProgressGraphComp