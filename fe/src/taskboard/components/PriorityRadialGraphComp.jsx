...
import PriorityRadialGraph from './PriorityRadialGraph'
import Grid from "@mui/material/Grid2"
import { axiosInstance } from '../../hooks/useApiCall'

const PriorityRadialGraphComp = () => {
    const [loading, setLoading] = useState(true);
    const [data, setData] = useState([]);
    const fetchIssueStatues = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get("/taskboard-overview/issue-priorities");
    ...
    return (
        <Grid size={3} sx={{ borderRadius: "8px", bgcolor: "#fcfafa", boxShadow: "0px 4px 8px rgba(0, 0, 0, 0.1)", marginY: "auto", padding: "5px" }}>
            ...
            <PriorityRadialGraph data={data}/>
        </Grid>
    )
}

export default PriorityRadialGraphComp