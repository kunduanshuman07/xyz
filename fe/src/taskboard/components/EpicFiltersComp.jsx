...
const EpicFiltersComp = () => {
    const { epicFilters, setEpicFilters } = useAssigneeFilter();
    const {epics, epicLabels} = useTaskboard();
    ...
    return (
        <Box display={'flex'}>
            <FormControl sx={{ width: 200, marginY: "auto", marginX: "10px" }} size='small'>
                <InputLabel id="demo-multiple-checkbox-label" sx={{ fontSize: "12px", fontFamily: "montserrat" }}>Epics</InputLabel>
                <Select
                    ...
                    renderValue={(selected) => (
                        ...
                        >
                            {selected.map((value) => (
                                <Chip key={value} label={epicLabels[value]} sx={{ fontFamily: "montserrat", fontSize: "8px", height: "18px" }} />
                            ))}
                        </Box>
                    )}
                    MenuProps={MenuProps}
                >
                    {epics?.map((name) => (
                        <MenuItem key={name.id} value={name.id} sx={{ fontSize: "10px" }}>
                            <Checkbox checked={epicFilters.includes(name.id)} size='small' checkedIcon={<AssignmentTurnedInIcon />} />
            ...
            {epicFilters?.length !== 0 &&
                <Tooltip title='Clear assignee filter'>
                    <IconButton sx={{ margin: "auto 5px" }} size='small' onClick={() => setEpicFilters([])}>
...
export default EpicFiltersComp;